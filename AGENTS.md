...
1. The **remote work directory** is the approved per-host execution area used by
   the adaptation containers and remote commands for source checkouts, temporary
   diagnostics, runs, caches, and bug reproducers.
...
- Keep every new remote write under the approved root, using these ordered names:
  `01-repos/` for checkouts, `02-environment/` for collection and runtime
  configuration, `03-issues/` for one-off diagnosis, `04-acceptance/` for
  evaluation tools/configuration, `05-runs/` for run output, `06-tmp/` and
  `07-cache/` for temporary or implicit writes, and `08-bugs/` for minimal
  operator reproducers. Keep a reproducer in `08-bugs/` and run it through its
  verified container path; do not duplicate it under `/bug`. Only when a required
  tool or upstream workflow specifically needs `/bug` may the same `08-bugs/`
...
- Set an explicit `cwd`/`workdir` for every remote command and stop on a failed
  ...
  move them, change mounts, or stop/restart the adaptation container to arrange
  the workspace.
...
- Treat plugin changes as maintainable contributions to a multi-model,
  multi-platform framework. Before editing, read the target checkout's design,
